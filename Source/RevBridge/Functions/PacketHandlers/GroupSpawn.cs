...
{
    internal class GroupSpawn
    {
        private byte spawnType;
        private ushort spawnAmount;
        private Packet packet;

        //_BEGIN -> Create temporary packet
        public void Begin(Packet _packet)
        {
            spawnType = _packet.ReadByte();
            spawnAmount = _packet.ReadShort();
            packet = new Packet(0x3019);
        }

        public void Data(Packet _packet)
        {
            packet.WriteByteArray(_packet.GetBytes());
        }

        //[MethodTimer.Time]
        public void End(Definitions.Character Character)
        {
            try
            {
                packet.Lock();

                for (int ix = 0; ix < spawnAmount; ix++)
                {
                    if (spawnType == 0x01)
                    {
