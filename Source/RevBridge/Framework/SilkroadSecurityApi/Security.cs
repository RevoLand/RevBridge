    ...
    public sealed class Security
    {
        ...
        private void GenerateSecurity(SecurityFlags flags)
        {
            ...
            if (m_security_flags.blowfish == 1)
            {
                ...
                response.WriteLong(m_initial_blowfish_key);
            }
            if (m_security_flags.security_bytes == 1)
            {
                ...
                response.WriteInt((int)m_seed_count);
                response.WriteInt((int)m_crc_seed);
            }
            if (m_security_flags.handshake == 1)
            {
                ...
                response.WriteLong(m_handshake_blowfish_key);
                response.WriteInt((int)m_value_g);
                response.WriteInt((int)m_value_p);
                response.WriteInt((int)m_value_A);
            }
        ...
        private void Handshake(ushort packet_opcode, PacketReader packet_data, bool packet_encrypted)
        {
            ...
            if (m_client_security)
            {
                ...
                response.WriteLong(m_challenge_key);
                m_outgoing_packets.Add(response);
            }
            else
            {
                ...
                if (flags.handshake == 1 && flags.handshake_response == 0)
                {
                    ...
                    // Handshake challenge
                    Packet response = new Packet(0x5000);
                    response.WriteInt((int)m_value_B);
                    response.WriteLong(m_client_key);
        ...
        public void AddEncryptedOpcode(ushort opcode)
        {
            lock (m_class_lock)
            {
                if (!m_enc_opcodes.Contains(opcode))
                {
